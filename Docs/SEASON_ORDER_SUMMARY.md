...
> Объединяет два документа: этот обзор листа и бывший
> `Docs/SEASON_ORDER_SUMMARY_KM_LOGIC.md` (детальная логика колонок «КМ:»,
> удалён при слиянии — см. раздел «Логика колонок «КМ:»» ниже).
...
| КМ: … (4 колонки) | см. раздел «Логика колонок «КМ:»» ниже — подробная логика, формулы в коде, расхождения с ТЗ |
...
Кратко по «КМ:»: счётчик идёт по **строкам RATING** (не по уникальным табельным); **«КМ: без 2 заказов в группе»** и **«КМ: все ограничения кроме исчерпания»** в текущем коде **совпадают**. Подробности — ниже.
...
После правки обновить этот документ.
