...
> **Статус на 2026-09-26:** рефакторинг `Docs/REFACTORING_REVIEW_2026-09-26.json` (этапы 0–4) реализовал часть пунктов ниже иначе, чем здесь предложено — читать оба документа вместе:
> - **P0-1** («быстрый Excel», отключить выравнивание данных) — сделано **иначе**: копирование индекса стиля вместо поячейкового присваивания (**PERF-01**, вид не меняется) и отдельный потоковый писатель **`src/excel_write_only.py`** за флагом `performance.excel_writer: "write_only"` (**PERF-07**, этап 06 на PROM: 26 → 17 с, пик памяти 2,0 ГБ → ~0,5 ГБ); `skip_data_alignment_sheets` (P0-1a) — как и было, флагом.
> - **P1-1** (одна сессия openpyxl для матрицы RATING, без второго `load_workbook`) — **не сделано**, `rating_item_matrix.apply_rating_item_matrix_colors` по-прежнему открывает книгу повторно; остаётся открытым.
> - **P0-2/P0-3** (архив SQLite: батчинг, фаза в `debug_phase`), **P1-2/P1-3, P2-*, P3-*** — не затронуты рефакторингом, актуальны как есть.
> - Новое, не описанное здесь: коды возврата, безопасный вывод в консоль Windows, детерминированный порядок строк/колонок SUMMARY, `collect_summary_keys` на словарях (этап 05 на PROM: 4,3 → ~0,5 с) — см. отчёт.
...
| **P0-1a** | **`skip_data_alignment_sheets`** (реализовано, v1.7.54): fnmatch-список листов без Alignment на данных; дефолт LIST-REWARDS / STATISTICS / RATING* / ORDER* | **замер 30.07.2026:** этап 06 **41.6→30.8 с (−26%)**; `_format_sheet` **23.3→13.0 с (−44%)**; Σ этапов **47.3→36.6 с (−23%)** | сделано | `CONFIG_RUN_INPUT.json` → `performance.skip_data_alignment_sheets`; `sheet_skips_data_alignment` |
