...
| **P0-1a** | **`skip_data_alignment_sheets`** (реализовано, v1.7.54): fnmatch-список листов без Alignment на данных; дефолт LIST-REWARDS / STATISTICS / RATING* / ORDER* | **−15% … −30% этапа 06** (замер на прогоне) | сделано | `CONFIG_RUN_INPUT.json` → `performance.skip_data_alignment_sheets`; `sheet_skips_data_alignment` |
