...
- **`input_archive_sqlite`** — опциональная запись сырых входных CSV в локальный файл SQLite после чтения (без отдельного сервера). Ключи: **`enabled`**, **`db_path`** (по умолчанию относительно корня проекта **`OUT/DB/spod_input_archive.sqlite`** — каталог **`OUT/DB`** создаётся при необходимости; **`OUT/`** в репозиторий не коммитится), флаги **`input_files[].archive_to_db`**, **`use_sha256_for_identity`**, сводка **`archive_file_inventory`** и снимки **`archive_file_snapshot`**. Подробно: **`Docs/INPUT_ARCHIVE_SQLITE_DESIGN.md`** и **`README.md`** (история версий **1.7.21**, **1.7.22**, **1.7.23**).
