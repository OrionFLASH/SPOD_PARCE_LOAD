...
- **`run_blocks`** — `PROM` / `IFT` / `PSI`. **`run_outputs`** — массив или объект по блокам. **`run_blocks_parallel`**. Вход: **`IN/<BLOCK>/{SPOD,FILE,POST,JS}/`**. Выход: **`OUT/<BLOCK>/YYYY/DD-MM/`**. Архив: **`OUT/DB/<BLOCK>/…_{BLOCK}….sqlite`**. Разбор — **`parse_run_blocks_config`**, **`parse_run_outputs_for_block`**, **`parse_input_files_by_block`**.
