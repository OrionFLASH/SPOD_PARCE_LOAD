...
- **`run_blocks`** — массив блоков **`PROM`** / **`IFT`** / **`PSI`**: какие наборы обрабатывать (по умолчанию `["PROM"]`). **`input_files`** — объект разделов `PROM`/`IFT`/`PSI` с полным списком файлов в каждом. Выход: **`OUT/<BLOCK>/YYYY/DD-MM/`**, имена **`SPOD_<BLOCK> …`**. Разбор — **`parse_run_blocks_config`**, **`parse_input_files_by_block`**.
