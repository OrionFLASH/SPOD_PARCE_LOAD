...
- `SPOD_CONSISTENCY_CHECKS_SQL_MIRROR.sql` — зеркало части `consistency_checks.rules` в одном запросе (Hive/Spark: `WITH`, `RLIKE`, `CONCAT_WS`). Не вызывается из Python. Результат: блок **SUMMARY** (`passed` 1/0, `violation_count` по каждому `rules[].id`) и блок **DETAIL** (строки нарушений); в комментариях у каждой проверки указаны **id** и **name** из `config.json` и связь с `src/consistency_checks.py`. Таблицы — плейсхолдеры `spod_dq.t_*` (заменить под витрину). В SQL нет правил `json_*`, отключённых `ref_contest_data_indicator` / `ref_group_indicator` и `csv_columns_count` — см. хвост файла.
