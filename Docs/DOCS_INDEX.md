...
- `INPUT_ARCHIVE_SQLITE_DESIGN.md` — опциональный архив сырых входных CSV в SQLite (`config.input_archive_sqlite`, модуль `src/input_archive_sqlite.py`). Файл БД по умолчанию: **`OUT/DB/spod_input_archive.sqlite`** (см. **`db_path`** в `config.json`).
