...
- `MANAGER_STATS.md` — отдельная книга табельных: `sources`, фильтры EMPLOYEE (`SURNAME`, `POSITION_NAME`), итоговое исключение `employee_placeholder_exclusion`, `enrich_columns`, `run_outputs` (`manager_stats_only`), производительность enrich.
