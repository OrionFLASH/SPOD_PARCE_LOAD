...
- `MANAGER_STATS.md` — отдельная книга табельных: `sources`, фильтры EMPLOYEE, `employee_placeholder_exclusion`, `enrich_columns` (ФИО, ТБ/ГОСБ, коды ролей в рейтинге `exists+join`, Email, метрики RATING), `column_formats`, сводка `MANAGER_STATS_SUMMARY`, `run_outputs` (`manager_stats_only`), производительность enrich.
