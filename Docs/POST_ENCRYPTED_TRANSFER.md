...
| `bundle_manifest.txt` | Зашифрованный манифест «файл в POST → путь в проекте» |
| `pack_post_encrypted_program.py.txt` | Утилита упаковки (открытый текст, для архива) |
| `decrypt_post_program.py.txt` | Утилита расшифровки (открытый текст) |
| `КУДА_ПОЛОЖИТЬ_ФАЙЛЫ.txt` | Карта путей после расшифровки |

**Имена в POST:** к каждому файлу добавлен суффикс `.txt`; из имён убраны фрагменты вроде `_auto_js` (в POST `leaders_for_admin_auto_js.py` → `leaders_for_admin.py.txt`). Исходные имена восстанавливаются в **OUT/POST** по манифесту.
...
1. Создайте **`IN/POST/`** и положите туда все файлы из пересланного **POST/**.
