...
> (данные: [`catalog.json`](param_review_editor/catalog.json); Live Server / импорт; при `file://` — зеркало `catalog.js`).  
> Правки → «Сохранить JSON» (`catalog_…json`) → в чат: «примени каталог».
