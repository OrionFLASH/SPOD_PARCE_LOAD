...
Документ описывает полный формат конфигурации и вывода проверок на консистентность данных. Реализация в коде не выполняется до перечитывания и утверждения.
...
Комбинация полей на листе должна быть уникальной. Сюда же относятся все проверки, которые сейчас задаются в **check_duplicates**: по ключу ищутся дубли, в колонку пишется пометка (например «x2», «x3») или пусто.

Текущая реализация: колонка с именем `"ДУБЛЬ: " + "_".join(key_cols)`; значение — пусто или «xN» (N — число строк с данным ключом).

**Текущие правила из `check_duplicates` (config.json), которые в едином формате задаются как тип `unique`:**
...
### 2.5. Тип `field_length` (проверка длины полей — консистентность по ограничениям)

Проверка того, что длина значений в указанных полях удовлетворяет заданным ограничениям (оператор + лимит). Сейчас это задаётся в **field_length_validations**: по листу задаётся **result_column** и **fields** (поле → limit + operator).

Текущая структура в config:
...
В едином формате `consistency_checks` это можно задавать одним правилом на лист (или разбить на несколько правил по полям — по желанию). Пример одного правила на лист:
