...
| **key_transforms** | Опциональный объект «имя колонки из **key_columns**» → `{ "type": "left", "length": N }`. При **`groupby`** для этой колонки берутся **первые N символов** значения (алиасы типа: **`left`**, **`left_chars`**, **`prefix`**). Пример: «Дата создания» → `YYYY-MM-DD` из datetime. Алиас ключа: **`key_column_transforms`**. |
...
4. **`groupby`** по эффективному ключу (колонки из **`key_columns`**, с учётом **`key_transforms`**) и подсчёт кратности выполняются **только на подмножестве активных строк**; полученные метки «xN» или пусто записываются в колонку результата; для неактивных строк ячейка остаётся пустой.
...
Реализация: `src/consistency_checks.py` — функции **`_normalize_unique_scope_conditions`**, **`_unique_scope_mode`**, **`_unique_scope_mask`**, **`_unique_require_non_empty_mask`**, **`_unique_active_row_mask`**, **`_unique_key_transform_value`**, **`_unique_effective_key_frame`**, **`_run_unique_check`**, **`collect_unique_result`**.
...
#### Пример (LIST-REWARDS: составной ключ + первые 10 символов даты, блок PROM)

```json
{
  "id": "unique_list_rewards_tournament_reward_person_created10",
  "name": "В LIST-REWARDS нет дублей по составному полю Код турнира, Код награды, Табельный номер сотрудника, первые 10 символов Дата создания",
  "type": "unique",
  "enabled": true,
  "blocks": ["PROM"],
  "sheet": "LIST-REWARDS",
  ...
  "key_transforms": {
    "Дата создания": { "type": "left", "length": 10 }
  },
  "unique_scope_mode": "all",
  "unique_scope_conditions": [],
  "unique_scope_column": "",
...
Проверка ячейки как строки **SPOD-JSON** выполняется в два этапа. **(1) Сырая строка:** **внешняя обёртка** всей ячейки (если есть) — **только двойные кавычки `"`**; обёртка **одинарными `'…'` запрещена** (типичная ошибка выгрузки); голый `{…}` / `[…]` без обёртки допускается; симметрия внешней обёртки двойной кавычкой (если ячейка начинается с `"`, должна заканчиваться на `"`, и наоборот); снятие **BOM** (U+FEFF); удаление **Unicode-пробелов** вне блоков **`"""…"""`** (в т.ч. неразрывный пробел U+00A0 из Excel/CSV), чтобы после нормализации не ломался **JSON**; корень — объект **`{`** или массив **`[`**; **каждый ключ** объекта — строго **`"""имя"""`** (имя: латиница, цифры, `_`); **значение** при ключе **не** из **`numeric_value_keys`** — либо строка **`"""…"""`**, либо вложенный объект, либо массив; **массив** в кавычки не берётся, **строковые элементы** массива — в **`"""…"""`**, числа и `true`/`false`/`null` в элементах — **без** кавычек; для ключей из **`numeric_value_keys`** значение — **только** число или `true`/`false`/`null` **без** кавычек. **(2)** замена **`"""` → `"`**, снятие одной внешней пары кавычек при наличии, **`json.loads`**; затем контроль типов для **`numeric_value_keys`** (после разбора — число, не строка; собираются **все** нарушения по дереву) и для **`array_value_keys`** (ключ при наличии должен быть **массивом `[]`**, не скаляром; отсутствие ключа — не ошибка; пустой `[]` допускается).

**Особые случаи (явно распознаются и кратко описываются в колонке проверки):**

- **Внешняя обёртка `'…'`** — сообщение о необходимости двойных кавычек `"…"`.
...
- **Ключ из `array_value_keys` как скаляр** (например `"""seasonItem""": """SEASON_…"""` вместо `["""SEASON_…"""]`) — префикс **`array_value_keys:`**.
...
| `array_value_keys` | Список имён ключей, которые при наличии должны быть **массивом `[]`** (не скаляр). В проекте: **`helpCodeList`** (CONTEST_FEATURE), **`helpCodeList`** + **`seasonItem`** (REWARD_ADD_DATA). |
| `output.column_on_sheet` | Колонка с результатом (**OK** или текст ошибки). |

Реализация: **`src/json_spod_format_check.py`**. Примеры правил — в **`config/CONFIG_CHECKS.json`** (идентификаторы **`spod_json_*`**). Правила выполняются во всех режимах, где идут **consistency_checks** (**`main_only`**, **`consistency_only`**, **`full`** / полный набор `run_outputs`).
...
- **Консоль (терминал)** при запуске **`main.py`**: подробный **INFO** из логгера на **stdout** не выводится (уровень консоли — **WARNING** и выше). Краткая сводка для пользователя формируется модулем **`src/console_ui.py`**, функция **`print_consistency_summary(results, rules=…)`**, где **results** — список словарей, возвращаемый **`run_consistency_checks_and_attach_summary`**; **rules** — `consistency_checks.rules` из конфига (для русских названий типов и ключей). Вспомогательная функция **`consistency_check_line_parts`** в **`consistency_checks.py`** собирает лист, ключ/поля и суть проверки.
- Содержимое консольной сводки:
  1. Заголовок **«Консистентность»**.
  2. **Если нарушений нет** (`violations` суммарно 0): строка «Проблем не обнаружено: N проверок, нарушений — 0»; далее **подразделы по типу проверки** (русское имя из колонки **ТИП ПРОВЕРКИ**, число правил в скобках); в каждом подразделе — таблица **Лист | Ключ / поля | Проверка (name) | OK** (ширина колонок подстраивается под терминал).
  3. **Если есть нарушения:** строки **OK не выводятся**; сообщение «Найдены ошибки консистентности: …»; по каждому проблемному правилу — название и **check_id**, суть, лист и ключ/поля, колонка результата на листе, число нарушений и до **6** примеров из **sample** (остальное — лист **CONSISTENCY** и debug-лог).
- Если правила в конфиге не заданы или проверки не запускались, в консоль выводится соответствующее короткое сообщение.

Подробнее поведение консоли и история изменений — в корневом **`README.md`** (раздел про **`console_ui`**, п. 8 программы **main**, история версий **1.7.13–1.7.15**, **1.7.38**).
