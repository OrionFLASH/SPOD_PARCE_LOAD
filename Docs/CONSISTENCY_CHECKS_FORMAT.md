...
**Текст в колонке результата:** короткие формулировки с **путём к месту** (например `getCondition.rewards[0].amount`, `объект «[0]»`, `объект «filtered_attribute_condition[3]»`); без длинных фрагментов исходной строки и без номера позиции в типовых ошибках SPOD. Длинные сообщения обрезаются при записи в ячейку (**`_MAX_CELL_ERROR_LEN`** в коде, по умолчанию **12000** символов).
...
- В ячейках по строкам:
  ...
  - **json_spod_format**: «OK» или короткое сообщение (путь к полю, суть: тройные кавычки у ключа/значения, лишние `{}` вокруг одной строки в массиве, `""key""` вместо `"""key"""`, для **numeric_value_keys** — список путей с типом/значением).
