...
**SQL-зеркало (витрина / Hive / Spark):** файл **`Docs/SPOD_CONSISTENCY_CHECKS_SQL_MIRROR.sql`** дублирует логику перечисленных выше типов правил в виде одного запроса со сводкой и деталями; идентификаторы проверок совпадают с **`rules[].id`** в конфиге. Полное описание назначения и ограничений — в шапке этого SQL и в **`Docs/DOCS_INDEX.md`**.
