...
def debug_remaining_zeros():
    """Отладка оставшихся нулевых счетчиков"""
    ...
    sheets = {
        ...
        'CONTEST-DATA': pd.read_excel(latest_file, sheet_name='CONTEST-DATA'),
        'INDICATOR': pd.read_excel(latest_file, sheet_name='INDICATOR'),
        'REWARD': pd.read_excel(latest_file, sheet_name='REWARD'),
    ...
    for counter in problematic_counters:
        ...
        if 'CONTEST-DATA=>COUNT_CONTEST_CODE' in counter:
            source_sheet = 'CONTEST-DATA'
            source_key = 'CONTEST_CODE'
            dest_key = 'CONTEST_CODE'
        elif 'INDICATOR=>COUNT_CONTEST_CODE' in counter:
            source_sheet = 'INDICATOR'
            source_key = 'CONTEST_CODE'
            dest_key = 'CONTEST_CODE'
        elif 'REWARD=>COUNT_REWARD_CODE' in counter:
            source_sheet = 'REWARD'
            source_key = 'REWARD_CODE'
            dest_key = 'REWARD_CODE'
        elif 'TOURNAMENT-SCHEDULE=>COUNT_TOURNAMENT_CODE' in counter:
            source_sheet = 'TOURNAMENT-SCHEDULE'
