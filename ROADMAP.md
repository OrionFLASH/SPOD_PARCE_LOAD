...
Согласование: пункты **2**, **3**, **6**, **7** — реализованы (см. планы в `Docs/`). Пункт **8** — анализ разбиения конфига (реализация после выбора варианта). Пункт **16** — fill/fill-full (`Docs/PLAN_WEB_FILL.md`); код сделан, **пересборка examples отложена**.
...
**Статус:** код реализован; пересборка `common/web-fill/examples/*.json` **отложена** (нет исходных CSV в репозитории).
...
| 16.1 | Экспортер JSON: без заглушек; сверка CSV; пересборка `examples/` | [w] |
...
16.1: экспортер и тесты готовы; **пересборка `examples/*.json` отложена** (CSV PROM-SPOD нет в репозитории). После появления CSV: `python3 src/Tools/export_web_fill_examples_from_spod.py`.
