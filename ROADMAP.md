...
Отдельные наборы SPOD-CSV по средам: `IN/SPOD/PROM`, `IN/SPOD/IFT`, `IN/SPOD/PSI`.
Переключатель `run_blocks` (как `run_outputs`): какие блоки считать в одном запуске.
Выход: `OUT/<BLOCK>/YYYY/DD-MM/`, имена `SPOD_<BLOCK> …`.

| # | Задача | Статус |
|---|--------|--------|
| 6.1 | `run_blocks` в config + разбор в `config_loader` (по умолчанию `["PROM"]`) | [v] |
| 6.2 | Поле `block` у SPOD-записей `input_files`; три набора с одинаковыми именами файлов | [v] |
| 6.3 | Цикл расчёта по блокам; фильтр `input_files`; shared `FILE` без `block` | [v] |
