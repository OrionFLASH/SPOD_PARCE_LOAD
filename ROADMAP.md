...
**Документы:** [`Docs/PLAN_CONSISTENCY_JSON_WRAP_ARRAYS.md`](Docs/PLAN_CONSISTENCY_JSON_WRAP_ARRAYS.md) (план + чеклист вместе).  
...
| 26.33 | Кнопки конвейера внизу слева; компактная статистика справа + подсказки | [v] |
| 26.34 | Docs + архив (актуализация 26.33) | [v] |
| 26.35 | JSON: автозагрузка источника/ФИО по пути; сетка 1 строка; превью свёрнуто | [v] |
...
| 26.49 | XLSX: колонка табельного номера (`MANAGER_PERSON_NUMBER`) всегда пишется текстом (`t:"s"`, формат `@`) — независимо от того, числовым или текстовым был этот столбец в исходном файле; 20-значный ТН с ведущими нулями больше не зависит от формата источника | [v] |
| 26.50 | Docs + архив (актуализация 26.49) | [v] |
| 26.51 | Справочник ФИО вынесен из карточки турнира в общую модалку: кнопка «Справочник ФИО» на левой панели (активна, если есть хоть один турнир FIO), одна загрузка применяется сразу ко всем турнирам режима FIO; данные и JSON (`fio` как общий блок) не менялись — только UI, раньше выглядевший «per-турнир» | [v] |
| 26.52 | Docs + архив (актуализация 26.51) | [v] |
| 26.53 | Перф: `applyPackOrigin` не пересчитывал таблицу для листа целиком у ВСЕХ листов Excel-книги при каждой смене листа/угла (только у выбранного); при разборе книги таблица строится сразу только для первого листа. Найдено при проверке сценариев (не подтверждён как причина сообщённого сброса списка турниров — репро не найдено) | [v] |
| 26.54 | Docs + архив (актуализация 26.53) | [v] |
| 26.55 | Перф/память: `readTableFile` не считает и не хранит в пакете base64-копию свежезагруженного файла (`source_b64`) — она никогда не читалась (содержимое файлов в JSON не сохраняется), только занимала CPU/память на каждую загрузку CSV/Excel/таблицы ФИО. Подтверждённая вероятная причина сообщённой жалобы: браузер — Safari, ошибок в консоли нет — похоже на аварийную перезагрузку вкладки Safari по нехватке памяти на `file://`-странице (не JS-исключение, поэтому в консоли пусто), а не на баг в логике | [v] |
| 26.56 | Docs + архив (актуализация 26.55) | [v] |
| 26.57 | Даты SCHEDULE (старт/завершение/итоги) в турнире и JSON + статус на сегодня («не стартовал / активен / подведение итогов / пора закрывать / закроется сейчас»); периоды H1/H2 и «турнир месяца» по START_DT; шапка — 6 плашек на всю ширину со статистикой (+«Выгрузка»); «Сформировать ✓ готово»; XLSX-листы «ФИО» и «REPORT изменения»; превью раскрыты; «Путь к файлу» и автозагрузка по пути удалены; панели шире, фильтры компактнее; фикс: дубли ФИО не разрешались | [v] |
| 26.58 | Docs + архив (актуализация 26.57) | [v] |
| 26.59 | Статус по датам в списке турниров; дни до старта / до конца / до закрытия / просрочка; «Справочник ФИО» оранжевый, если есть FIO и справочник пуст; кнопка «Загрузить REPORT» для ручных турниров (обновляется именно он); трейс-лог (`report_trace.js`, переключатель «Лог», запись сразу в localStorage, скачивание всего журнала, отметка аварийного завершения прошлой сессии); кнопки конвейера — пары в две колонки | [v] |
| 26.60 | Docs + архив (актуализация 26.59) | [v] |
| 26.61 | Подсказки: авто-выбор стороны (сверху/снизу) и сдвиг внутрь окна — у шапки больше не уходят за край экрана; стиль — матовое полупрозрачное стекло со стрелкой и плавным появлением | [v] |
| 26.62 | Docs + архив (актуализация 26.61) | [v] |
| 26.63 | Фикс сброса страницы (по трейс-логу): Safari уходил «Назад» по истории (back_forward) — жест свайпа у края прокрутки после перерисовки; перехват «Назад» (pushState/popstate), подтверждение при закрытии/перезагрузке, overscroll-behavior, сохранение фокуса и прокрутки превью при перерисовке, колесо не меняет поле-число; трейс пишет свайпы/клавиши/историю | [v] |
| 26.64 | Docs + архив (актуализация 26.63) | [v] |
| 26.65 | Фикс: трейс-лог не выключался — запись о клике по переключателю обновляла UI и возвращала галочку во «вкл» до события change; «Очистить» режим не меняет (подсказка уточнена) | [v] |
| 26.66 | Docs + архив: пользовательский гид — сценарий 4 (REPORT из списков или вручную), сценарий 10 (включить/выключить/очистить лог), новый сценарий 11 (контроль сроков), «Перед началом» (защита данных), FAQ | [v] |
| 26.67 | Лист XLSX «REPORT изменения» при загруженном REPORT есть всегда: турнир без пришедших/ушедших — строка «БЕЗ ИЗМЕНЕНИЙ» (раньше пустой лист не записывался — казалось, что листа нет); трейс-лог при старте страницы всегда выключен, оборванная прошлая сессия всё равно отмечается в журнале | [v] |
