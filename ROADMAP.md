...
Согласование: пункты **2**, **3**, **6**, **7** — реализованы (см. планы в `Docs/`). Пункт **8** — анализ разбиения конфига (реализация после выбора варианта). Пункт **16** — план fill/fill-full (`Docs/PLAN_WEB_FILL.md`); **код не начат**.
...
**Страницы:** `common/web-fill/`, `common/web-fill-full/`. Каталог — через `web-edit` + `sync_web_fill_catalog.py`.  
...
| 16.1 | Экспортер JSON: без заглушек; сверка CSV; пересборка `examples/` | [ ] |
| 16.2 | Импорт fill: пустые массивы не порождают `t_/r_` и `emptyScheduleRow` | [ ] |
| 16.3 | Скрытие левой панели: колонтитулы остаются | [ ] |
| 16.4 | Правая панель поиска/фильтров, независимое скрытие | [ ] |
| 16.5 | Фильтры ПРОМ/ТЕСТ, `TOURNAMENT_STATUS`, дата в `[START_DT, END_DT]` | [ ] |
| 16.6 | Легенда цветов (столбик приоритета, не матрица) | [ ] |
| 16.7 | Каталог edit+fill: списки/подписи + combobox `INDICATOR_CODE` | [ ] |
| 16.8 | Sync каталога, README fill, приёмка fill ≡ fill-full | [ ] |
