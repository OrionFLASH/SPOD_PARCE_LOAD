...
from src.main_impl import _parse_date_column_to_date, add_fields_to_sheet, calculate_tournament_status
...
def test_add_fields_empty_after_filter_is_info_not_warning(caplog) -> None:
    """После фильтра 0 строк — INFO с понятным текстом, не WARNING «лист пустой»."""
    ...
    assert any("После фильтрации" in m and "было 292" in m for m in msgs)
    assert not any("пустой или None" in m for m in msgs)
    warn_msgs = [r.getMessage() for r in caplog.records if r.levelno >= logging.WARNING]
    assert not any("TOURNAMENT-SCHEDULE" in m for m in warn_msgs)
