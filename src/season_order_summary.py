# -*- coding: utf-8 -*-
"""
Сводный лист заказов по группам сезона (SEASON_*) из config item_order_groups.

Строит таблицу по кодам ITEM: заказано по ORDER, остаток itemAmount, признак «ЗАКОНЧИЛСЯ»,
...
def _format_threshold(val: Optional[float]) -> str:
    if val is None:
        return ""
    try:
        f = float(val)
...
def build_season_order_summary_sheet(
    ...
    """
    Формирует лист сводки по кодам из item_order_groups.
    ...
    for name, key in ((sr, "rating"), (so, "order"), (rw, "reward")):
    ...
    groups = rim._parse_item_order_groups(cfg)
    if not groups:
        logging.warning("[season_order_summary] item_order_groups пуст — нечего сводить")
        return None

    code_to_group = _code_to_group_map(groups)
    group_map = _group_by_id(groups)
    codes_sorted = sorted(code_to_group.keys())
    ...
    for code in codes_sorted:
        ...
        grp = group_map.get(group_id) or {}
        try:
            max_group_orders = int(grp.get("max_orders") or 0)
        except (TypeError, ValueError):
            max_group_orders = 0
        ...
        if limit_amt is not None:
            remainder = limit_amt - ordered_total
            total_display: Any = limit_amt
        else:
            remainder = ""
            total_display = ""
        ...
        for m in managers:
            ...
            blocked = rim._blocked_codes_for_row(count_by_code, [grp]) if grp else set()
        ...
        result_rows.append(
            {
                ...
                "Всего товаров": total_display,
                "Заказано": ordered_total,
                "Остаток": remainder if remainder != "" else "",
                "Статус наличия": status_cell,
                "Мин. рейтинг BANK": _format_threshold(mn.get("minRatingBANK")),
                "Мин. рейтинг TB": _format_threshold(mn.get("minRatingTB")),
                "Мин. рейтинг GOSB": _format_threshold(mn.get("minRatingGOSB")),
                "Мин. кристаллов": _format_threshold(er.get("minCrystalEarnedTotal")),
                ...
                "ignoreConditions (кол-во)": len(ignore_tabs) if ignore_tabs else "",
    ...
    logging.info(
        "[season_order_summary] Лист «%s»: %s строк (кодов в группах SEASON)",
        sheet_out,
        len(df_out),
    )
