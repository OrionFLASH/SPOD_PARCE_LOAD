...
- **Смысл (гипотеза):** число знаков после запятой для отображения.
...
- **Смысл (гипотеза):** уменьшение отображаемого показателя до млн / до тыс.

##### `feature`

- **Тип:** массив строк (особенности турнира).
...
- **Смысл (гипотеза):** коды для вывода окна с доп. описанием конкурса.
...
- **Смысл (гипотеза):** мин. число участников чтобы считать победителей (исключаем соревнование сам с собой); обычно `1` | `2` | `3`.

##### `momentRewarding`

- **Тип:** строка (момент начисления награды).
- **CONTEST_TYPE:** все.
- **Домен:** **`AFTER`** (после закрытия турнира); **`DURIN`** — во время турнира (см. примечание про опечатку). У **ТУРНИРНЫЙ** в выборке только **`AFTER`**.
- **Смысл (гипотеза):** момент награждения — после закрытия / во время турнира.

##### `persomanNumberHidden`

- **Тип:** массив; в выборке **всегда пустой**.
- **CONTEST_TYPE:** все.
- **Смысл (гипотеза):** если указаны табельные — эти сотрудники НЕ увидят турнир.
...
- **Смысл (гипотеза):** если указаны табельные — только эти сотрудники увидят турнир.

##### `preferences`

- **Тип:** массив строк.
- **CONTEST_TYPE:** только **ТУРНИРНЫЙ**; **15** строк; 0–4 элемента; тексты **длинные** (до ~158 символов).
- **Смысл (гипотеза):** преференции за получение награды, если предусмотрены.
...
- **Смысл (гипотеза):** вручаем одну из 3 наград или все.
