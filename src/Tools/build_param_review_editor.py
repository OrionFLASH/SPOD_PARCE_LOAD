# -*- coding: utf-8 -*-
"""Собрать Docs/param_review_editor/catalog.js из схемы + field_meta + MD-каталога."""
...
def main() -> None:
    ...
    (OUT_DIR / "catalog.json").write_text(payload + "\n", encoding="utf-8")
    total = sum(len(s["fields"]) for s in catalog["sections"])
    print(f"OK: {js_path} ({total} полей, {len(catalog['sections'])} секций)")
