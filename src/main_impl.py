...
def _load_config_globals():
    ...
    global ALL_INPUT_FILES, RUN_BLOCKS, CURRENT_RUN_BLOCK
    ...
    try:
        ...
        if _c is not None:
            ...
            ALL_INPUT_FILES = list(_c.input_files)
            RUN_BLOCKS = list(getattr(_c, "run_blocks", None) or ["PROM"])
            CURRENT_RUN_BLOCK = RUN_BLOCKS[0] if RUN_BLOCKS else "PROM"
            INPUT_FILES = filter_input_files_for_block(ALL_INPUT_FILES, CURRENT_RUN_BLOCK)
    ...
    ALL_INPUT_FILES = list(_cfg["input_files"])
...
def apply_run_block_context(block: str) -> None:
    """
    Переключает контекст прогона на блок: фильтр input_files и имена выходных файлов.
    """
