...
Каталоги параметров (`catalog.json`, `game_edit_catalog.json`) содержат **`marks: ["ПКАП","ФАБРИКА"]`** у каждого поля (после `allow_empty`). Обогащение: `python src/Tools/enrich_catalog_marks.py`.
