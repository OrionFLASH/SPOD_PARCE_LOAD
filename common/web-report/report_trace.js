 ...
 * вкладки журнал не теряется; после перезагрузки запись продолжается в тот же
 * журнал с пометкой, завершилась ли прошлая сессия штатно.
...
(function (root) {
  ...
  enabled = safeGet(KEY_ON) === "1";
  cache = safeGet(KEY_LOG) || "";
  if (enabled) startSession("загрузка страницы");
