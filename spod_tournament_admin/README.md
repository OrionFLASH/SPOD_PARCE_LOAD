...
- **Редактор строки (клиент):** `src/static/row_editor.js` — читает JSON из `#row-editor-bootstrap` (формирует сервер в `app.row_detail`), строит сетки полей, собирает JSON обратно при сохранении; при ошибке разбора колонки доступен только сырой текст; сравнивает текущее состояние формы с начальным и включает кнопки «Сохранить»/«Отменить» только при наличии отличий.
...
| `app.index`, `sheet_list`, `row_detail` | HTML-страницы; `sheet_list` обогащает строки через `sheet_list_display`; в `row_detail` передаётся `editor_bootstrap_json` для скрипта |
