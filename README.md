...
**Логика:** для каждой строки листа проверяется длина указанных полей. Если длина не соответствует правилу (например, не равна 20 при `operator: "="`), в `result_column` записывается признак ошибки; иначе — признак «OK». Используется для контроля качества данных.
...
**Логика:** для листа по полям `key` считается количество повторений комбинации; если больше 1, в новой колонке «ДУБЛЬ: ...» ставится признак дубликата. Одна запись в конфиге — одна такая колонка (на одном листе может быть несколько правил с разными ключами).
