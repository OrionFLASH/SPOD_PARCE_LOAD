...
| `consistency_checks` | **Единый конфиг всех проверок консистентности:** сводный лист (summary_sheet_name), массив правил (rules) с типами referential, referential_composite, **unique**, **field_length**, **field_format** (формат даты/числа/строки). Модуль src/consistency_checks.py создаёт колонки на листах и собирает результаты в лист CONSISTENCY (с колонками-описаниями по образцу таблицы проверок: ТИП ПРОВЕРКИ, Описание, таблица источник и т.д.), лог и консоль. Секции `check_duplicates` и `field_length_validations` в config.json **удалены** — все правила задаются здесь. |
